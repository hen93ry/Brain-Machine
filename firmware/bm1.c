...
//   table of values for entrainment
//   Gamma 29hz (attentive unkwown usage/makes my eyes cross 30hz-100hz)
//   Gamma/Beta (bridge 23 hz)
//   Beta 16.7hz (awake/conscious 13 hz-30hz)
//   Beta/Alpha (bridge 13.7hz)
//   Alpha  10.8hz (receptive/passive 8hz-13hz)
//   Alpha/Theta (bridge 8hz)
//   Theta 5.3hz (Creativity/deep relaxing 4hz-8hz)
//   Theta/Delta (bridge 4hz) 
//   Delta 2.6hz (intuitive/insight 0.5-4hz) 

struct brainwaveElement {
  char bwType;  // 'g' for Gamma,'w' for Gamma/Beta, 'a' for Alpha, 'z' for Alpha/Beta 'b' for Beta, 'y' for Alpha/Theta,'t' for Theta, 'x' for Theta/Delta or 'd' for Delta ('0' signifies last entry in table
...
/*
      ...
      // start Timer1 with the correct Offset Frequency for a binaural beat for the Brainwave Type
      //   to Right ear speaker through output OC1A (PB1, pin 6)
      OCR1C = 67;  // T1 generates 229.77941Hz, for a binaural beat of 29.5Hz
      // delay for the time specified in the table while blinking the LEDs at the correct rate
      //   onTime = 16.9ms, offTime = 17.0ms --> 29.45Hz
      blink_LEDs( pgm_read_dword(&brainwaveTab[index].bwDuration), 169, 170 );
      return;   // Gamma
    }
      ...
      // start Timer1 with the correct Offset Frequency for a binaural beat for the Brainwave Type
      //   to Right ear speaker through output OC1A (PB1, pin 6)
      OCR1C = 69;  // T1 generates 223.21428Hz, for a binaural beat of 22.89Hz
      // delay for the time specified in the table while blinking the LEDs at the correct rate
      //   onTime = 21.8ms, offTime = 21.8ms --> 21.83Hz
...
*/
      ...
      // start Timer1 with the correct Offset Frequency for a binaural beat for the Brainwave Type
      //   to Right ear speaker through output OC1A (PB1, pin 6)
      OCR1C = 72;  // T1 generates 213.3205Hz, for a binaural beat of 13Hz
      // delay for the time specified in the table while blinking the LEDs at the correct rate
      //   onTime = 36.4ms, offTime = 36.4ms --> 13Hz
      blink_LEDs( pgm_read_dword(&brainwaveTab[index].bwDuration), 364, 364 );
      return;   // Alpha/Beta
    }
      ...
      // start Timer1 with the correct Offset Frequency for a binaural beat for the Brainwave Type
      //   to Right ear speaker through output OC1A (PB1, pin 6)
      OCR1C = 74;  // T1 generates 208.3005Hz, for a binaural beat of 7.98Hz
      // delay for the time specified in the table while blinking the LEDs at the correct rate
      //   onTime = 62.3ms, offTime = 62.4ms --> 5.3Hz
      blink_LEDs( pgm_read_dword(&brainwaveTab[index].bwDuration), 623, 624 );
      return;   // Alpha/Theta
    }
      ...
      // start Timer1 with the correct Offset Frequency for a binaural beat for the Brainwave Type
      //   to Right ear speaker through output OC1A (PB1, pin 6)
      OCR1C = 75.5;  // T1 generates 204.2915Hz, for a binaural beat of 3.971Hz
      // delay for the time specified in the table while blinking the LEDs at the correct rate
      //   onTime = 1272ms, offTime = 1273ms --> 3.971Hz
      blink_LEDs( pgm_read_dword(&brainwaveTab[index].bwDuration), 1272, 1273 );
      return;   // Theta/Delta
    }
...
int main(void) {
  ...
  //  each Brainwave Element consists of a Brainwave Type (Beta, Alpha, or etc..) and a Duration
